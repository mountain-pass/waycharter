...
if [ -z "$RUN_ID" ]; then
  echo "Could not find workflow run. Check https://github.com/mountain-pass/waycharter/actions"
  exit 1
fi

echo "Build workflow: https://github.com/mountain-pass/waycharter/actions/runs/$RUN_ID"
echo ""
...
if gh run watch "$RUN_ID" --exit-status; then
  ...
  echo ""
  echo "Pipeline failed. Check: https://github.com/mountain-pass/waycharter/actions/runs/$RUN_ID"
  exit 1
fi
...
elif [ "$PUBLISH_CONCLUSION" = "skipped" ]; then
  echo ""
  echo "Publish job was skipped."
else
  echo ""
  echo "Publish job conclusion: ${PUBLISH_CONCLUSION:-unknown}"
fi
