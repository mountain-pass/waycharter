...
/* istanbul ignore next: branch only taken when retry is enabled */
const RETRY = process.env.RETRY ? `--retry ${process.env.RETRY || 5}` : ''
...
function generateConfig() {
  ...
  const CONFIG = `${FEATURE_GLOB} --format-options '${JSON.stringify(
    FORMAT_OPTIONS
  )}' ${MODULES} ${REQUIRE_GLOB} ${NO_STRICT} --format rerun:${RERUN} ${RETRY} ${PUBLISH} ${FAIL_FAST}`
